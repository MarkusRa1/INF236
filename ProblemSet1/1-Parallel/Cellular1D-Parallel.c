...
void writeToFile(char *fileName, int *history, int w, int h, int *sendcount, int comm_sz);
int mod(int x, int m);
...
int main(int argc, char **argv)
{
    ...
    // }
    int *gathersendcount = malloc(comm_sz * sizeof(int));
    for(size_t i = 0; i < comm_sz; i++)
    {
        gathersendcount[i] = sendcount[i] * (numOfIt + 1);
        displays[i] = displays[i] * (numOfIt + 1);
    }
    
    MPI_Gatherv(history, sendcount[my_rank] * (numOfIt + 1), MPI_INT, historyall, gathersendcount, displays, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (my_rank == 0)
    {
        ...
        // printMatrix(historyall, myCellsSize, (numOfIt + 1)*2);
        // printMatrix(history, myCellsSize, (numOfIt + 1));
        // writeToFile("de", historyall, cellsSize, numOfIt + 1, sendcount, comm_sz);
    }
    return 0;
}
...
void writeToFile(char *fileName, int *history, int w, int h, int *sendcount, int comm_sz)
{
    ...
    int *proclocs = malloc(comm_sz * sizeof(int));
    for(size_t proc = 0; proc < comm_sz; proc++)
    {
        if (proc == 0) {
            proclocs[proc] = 0;
        } else
        {
            proclocs[proc] = proclocs[proc-1] + sendcount[proc-1]*h;
        }
    }
    printArray(proclocs, comm_sz);
    ...
    for (size_t i = 0; i < h; i++)
    {
        for (size_t proc = 0; proc < comm_sz; proc++)
        {
            ...
            for (size_t j = 0; j < sendcount[proc]; j++)
            {
                fprintf(fp, "%d,", *(history + proclocs[proc] + i * sendcount[proc] + j));
            }
        }
        // printf("%d\n", *(history + i*h + w-1));
        fprintf(fp, "%d\n", *(history + i * w + w - 1));
